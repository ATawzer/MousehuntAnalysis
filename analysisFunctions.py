...
def GeneralAnalysis(df):
    ...
    print('Global Catch Rate: ' + "{:,.2f}".format(round(np.sum(df.totalCatches)/np.sum(df.totalAttractions), 2)) +
          "% | Global Attraction Rate: " + "{:,.2f}".format(round(np.sum(df.totalAttractions)/np.sum(df.totalHunts), 2)) + '%\n')
...
def allRunStats(df):
    ...
    for index, row in df.iterrows():
        for col in cols:
            if col != "RunName" and col != "Location":
                row[col] = "{:,.2f}".format(row[col])
    df.set_index('RunName', inplace=True)
